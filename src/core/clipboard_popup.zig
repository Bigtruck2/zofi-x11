...
const default_height: c_uint = 510;
...
const max_preview_bytes: usize = 120;
...
pub const Selector = struct {
    ...
    pub fn selectedEntryIndex(self: *const Selector) ?usize {
...
//var fds = [_]x11.pollfd{.{
    ...
    fn handleKeyPress(self: *Popup, key_event: *x11.XKeyEvent) !bool {
        ...
        if (keysym == x11.XK_Return or keysym == x11.XK_KP_Enter) {
            if (self.selector.selectedEntryIndex()) |entry_index| {
                try clipboard.copy(self.init_state, self.entries[entry_index].text);
                return true;
            }
            return false;
        }
    ...
    fn rerank(self: *Popup) !void {
        ...
        for (self.entries, 0..) |entry, entry_index| {
            if (!containsIgnoreCase(entry.text, self.selector.query.items)) continue;
            try self.selector.matches.append(self.allocator, entry_index);
        }
    ...
    fn preview(text: []const u8) []const u8 {
        ...
        return line[0..@min(line.len, max_preview_bytes)];
    }
    ...
    pub fn redraw(self: *Popup) void {
        ...
        if (self.cursor_visible) {
            const glyph_width: c_int = 12;
            const cursor_x = query_x + @as(c_int, @intCast(query.len)) * glyph_width + 2;
        ...
        for (0..max_visible_results) |visible_row| {
            ...
            const entry_index = self.selector.matches.items[result_index];
            ...
            self.drawText(padding, row_y + 39, preview(entry.text));
        }
